/*========================================================*/
/*                                                        */
/*  pfitsin.c       version 3.8.0         2015.10.07      */
/*                                                        */
...
float *read_FITS_1Dfile(char *iname, int *ncards, char ***header, int *n1)
{
  ...
  switch(bitpix)
  {
    case   8: for (i=0; i<nx; i++)  ldata[i]=(float)((uint8_t *)buf)[i];
              break;
    case  16: for (i=0; i<nx; i++)  ldata[i]=(float)((int16_t *)buf)[i];
              break;
    case  32: for (i=0; i<nx; i++)  ldata[i]=(float)((int32_t *)buf)[i];
              break;
  ...
  int *n1, int *n2)
{
  ...
  switch(bitpix)
  {
    case   8: for (i=0; i<ny; i++)
                for (j=0; j<nx; j++)
                  ldata[i][j]=(float)((uint8_t *)buf)[i*nx+j];
              break;
    case  16: for (i=0; i<ny; i++)
                for (j=0; j<nx; j++)
                  ldata[i][j]=(float)((int16_t *)buf)[i*nx+j];
              break;
    case  32: for (i=0; i<ny; i++)
                for (j=0; j<nx; j++)
                  ldata[i][j]=(float)((int32_t *)buf)[i*nx+j];
              break;
    case -32: for (i=0; i<ny; i++)
                for (j=0; j<nx; j++)
